...
/** POST — body: { flashcards: { question: string, answer: string }[] }. Returns { message: string }. */
export async function POST(req: Request) {
  ...
  const flashcardLines = rawFlashcards
  ...
  const formattedFlashcardsString = flashcardLines.join("\n\n");
  ...
  try {
    const completion = await client.chat.completions.create({
      ...
      messages: [
        ...
        {
          role: "user",
          content: formattedFlashcardsString,
        },
      ],
    });
    ...
    console.error("POST /api/simulation-start:", error);
    return NextResponse.json(
